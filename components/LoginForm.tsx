...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
      ...
      <path
        fill="#FBBC05"
        d="M6.4 13.8a6 6 0 0 1 0-3.6V7.7H3.1a10 10 0 0 0 0 8.9l3.3-2.8Z"
      />
      <path
        fill="#EA4335"
    ...
    <div className="flex flex-col justify-between gap-8 bg-white px-6 py-8 sm:px-8 lg:px-10 lg:py-12">
      <div className="space-y-8">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          ...
          <p className="text-sm leading-6 text-slate-gray">
            Use your workspace credentials to continue to Webluma.
          </p>
          ...
          <div className="grid grid-cols-3 gap-3">
            {oauthOptions.map((option) => (
              <button
                key={option.name}
                type="button"
                aria-label={`${option.name} OAuth option, visual only`}
                title={`${option.name} OAuth option is visual only`}
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-mist-gray bg-white px-3 text-xs font-semibold text-midnight-slate shadow-[0_1px_0_rgba(15,23,42,0.02)] disabled:cursor-not-allowed disabled:opacity-100"
              >
                {option.icon}
